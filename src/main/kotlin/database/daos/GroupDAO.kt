...
class GroupDAO(url: String, dbProperties: Properties, tableName: String, connectionPool: ConnectionPool) : GenericDAO(url, dbProperties, tableName, connectionPool) {
    ...
    fun insertGroup(group: Group): Boolean {
        connect()
        try {
            val createGroupStatementString = "INSERT INTO $tableName (CREATED_DATE_TIME, LAST_UPDATED_DATE_TIME, GROUP_NAME, ID_PARENT_GROUP, DEFAULT_GROUP, HIDDEN) VALUES (?,?,?,?,?,?) ${DAOManager.getConflictConstraintCommand("groups_group_name_key")}"
